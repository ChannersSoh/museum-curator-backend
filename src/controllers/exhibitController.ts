...
const getHarvardObjects = async (query: string = ""): Promise<Exhibit[]> => {
  try {
...
const getSmithsonianData = async (query: string = ""): Promise<Exhibit[]> => {
  try {
...
const determineType = (typeString: string | undefined): Exhibit["type"] => {
...
const determineHistoricalEra = (date: string | undefined): string => {
...
export const getExhibits = async (req: Request, res: Response) => {
  try {
    ...
    console.log(`Fetching exhibits for query: "${searchQuery}"`);
    ...
    console.log(`Fetched ${combinedExhibits.length} exhibits`);
